"""
This application is a simple commandline program which takes a string input from a user, then generates a 
QR code and takes said code and converts it into a 3d model. 
"""
...
def main():
    ...
    parser.add_argument("-d", "--depth", type=float, default=0.5,
                         help="the base depth of the mesh, for cells which are considered to be false")
    parser.add_argument("-t", "--true-depth", type=float, default=1.0,
    ...
    if validate_args(args): 
        params: MeshConstructionParams = MeshConstructionParams(size=args.size, depth=args.depth, true_depth= args.true_depth)
